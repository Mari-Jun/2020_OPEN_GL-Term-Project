...
void BillBoardComponent::draw(std::unique_ptr<Shader>& shader)
{
	if (mTexture)
	{
		...
		shader->setMatrixUniform("uInvertView", mOwner.lock()->getGame().lock()->getRenderer()->getInvertViewMatrix());
		shader->SetVectorUniform("uTexSize", Vector3(static_cast<float>(mTexWidth), static_cast<float>(mTexHeight), 1.0f));
